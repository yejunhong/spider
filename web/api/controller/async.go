		...
		if resource.BookType == 1 { // 漫画
			...
			go func(){
				...
				for k, v := range bookList {
					...
					controller.ayncPortalChapter(v, chapterLists)
					fmt.Println("已同步：", k)
				}
			}()
		}
...
func(controller *Controller) ayncPortalChapter(book model.CartoonList, chapter []model.CartoonChapter) {
...
func (controller *Controller) ayncPortalCategory(book model.CartoonList, pid int64) {
    ...
    for _, v := range tags {
		...
		controller.Model.Db61.Where("post_id = ?", ChapterUniqueId).Delete(CmfPortalCategoryPost{})
    if len(data) > 0 {
...
func(controller *Controller) ayncXiaoShuo(book model.CartoonList, chapter []model.CartoonChapter) {
    ...
    for _, v := range chapter {
			...
			data = append(data, map[string]interface{}{
				...
				"update_ok": 1, // '更新到正式环境状态;1:已更新,0:未更新',
